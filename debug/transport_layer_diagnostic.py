...
class ArchitecturalTest:
    ...
    async def test_lightweight_wrapper_functionality(self):
        ...
        try:
            ...
            self.add_result("Lightweight Wrapper - Tools List", tools_success, f"Found {len(tools)} tools")
            ...
            if tools_success and tools:
                ...
                self.add_result(
                    "Lightweight Wrapper - Tool Call", tool_call_success, f"Tool result: {result.get('content', 'N/A')}"
                )

            # Test clean shutdown (the critical test!)
            try:
                await transport.close()
                self.add_result("Lightweight Wrapper - Clean Shutdown", True, "No cancel scope errors")
                return True
            except Exception as e:
                error_msg = str(e)
                is_cancel_scope_error = "cancel scope" in error_msg.lower()
                self.add_result("Lightweight Wrapper - Clean Shutdown", False, f"Error: {error_msg}")
    ...
    async def test_architecture_principles(self):
        ...
        try:
            ...
            self.add_result(
                "Architecture - Lightweight Implementation",
                is_lightweight,
        ...
        self.add_result(
            "Architecture - No Transport Reimplementation",
            no_reimplementation and good_pattern_count >= 2,
            "Uses delegation patterns, no direct transport implementation",
        )
    ...
    async def test_cancel_scope_fix(self):
        ...
        try:
            ...
            for _i in range(3):
                ...
                try:
                    ...
                    if "cancel scope" in error_msg.lower():
                        self.add_result(
                            "Critical Fix - No Cancel Scope Errors", False, f"Cancel scope error detected: {error_msg}"
                        )
                        return False
                    else:
            ...
            self.add_result(
                "Critical Fix - No Cancel Scope Errors",
                True,
            ...
            self.add_result("Critical Fix - No Cancel Scope Errors", not is_cancel_scope, f"Exception: {error_msg}")
