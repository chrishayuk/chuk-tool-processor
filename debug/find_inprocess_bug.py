    ...
    try:
        ...
        if len(lines) >= 533:
            ...
            if "wait_for" in line_533 and "10.0" in line_533:
                print("🚨 FOUND THE BUG! Line 533 has hardcoded 10.0:")
                print(f"   {line_533.strip()}")
            elif "wait_for" in line_533:
                print("ℹ️  Line 533 has wait_for but uses variable timeout")
                print("   Need to check what 'timeout' variable contains")
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if "wait_for" in line and ("10.0" in line or "30.0" in line) or "timeout" in line and ("= 10.0" in line or "= 30.0" in line):
...
def check_default_timeout_flow():
    ...
    try:
        ...
        async def trace_timeout_flow():
            ...
            print("🔍 Testing _run_with_timeout with timeout=2.0...")
            ...
            try:
                await strategy._run_with_timeout(
                    tool=mock_tool,
                    call=call,
                ...
                if captured_timeout == 2.0:
                    print("   ✅ Correct! _run_with_timeout used timeout=2.0")
                elif captured_timeout == 10.0:
                    print("   🚨 BUG! _run_with_timeout used timeout=10.0 instead of 2.0")
                    print("      This means there's a hardcoded 10.0 in the method!")
                else:
