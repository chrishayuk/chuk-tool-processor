...
class EnhancedMCPDuplicationTester:
    ...
    async def setup_test_config(self) -> str:
        ...
        config = {
            "mcpServers": {
                ...
                "sqlite": {
                    "command": "uvx", 
                    "args": ["mcp-server-sqlite", "--db", "/tmp/mcp_demo.sqlite"]
                },
                "time2": {
                    "command": "uvx",
    ...
    async def test_tool_name_collision(self) -> Dict[str, Any]:
        ...
        try:
            ...
            state1 = await self.get_registry_state("After first tool registration")
            ...
            collision_detected = (
                len(registered2) == 0 or  # Registration rejected
                state2["duplicate_counts"].get(namespace, {}).get("duplicate_tool", 0) > 1  # Duplicate created
            )
    ...
    async def test_concurrent_registration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            concurrent_tools = [
                [{"name": f"concurrent_tool_{i}", "description": f"Tool from source {i}"}] 
                for i in range(5)
            ]
            ...
            results = await asyncio.gather(*[
                register_concurrent(tools, i) for i, tools in enumerate(concurrent_tools)
    ...
    async def test_large_scale_registration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "tools_per_second": tools_per_second,
                "performance_grade": "excellent" if tools_per_second > 50 else "good" if tools_per_second > 20 else "needs_optimization",
                "final_state": final_state
            }
    ...
    async def test_error_recovery(self) -> Dict[str, Any]:
        ...
        try:
            ...
            state_after_failure = await self.get_registry_state("After failed registration")
    ...
    def analyze_enhanced_results(self, test_results: Dict[str, Any]):
        ...
        for test_name, result in test_results.items():
            ...
            if test_name == "basic_registration":
                ...
                print(f"   🛡️  Failure handling: {'✅ Graceful' if result['failure_handling']['failure_handled_gracefully'] else '❌ Poor'}")
