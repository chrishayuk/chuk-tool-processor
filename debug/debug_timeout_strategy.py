...
async def debug_timeout_handling():
    ...
    for config in timeout_configs:
        ...
        strategy = InProcessStrategy(registry, default_timeout=timeout)

        executor = ToolExecutor(registry=registry, strategy=strategy)

        # Create a test call
        test_call = ToolCall(
            tool="perplexity_search", namespace="sse", arguments={"query": f"Test query with {timeout}s timeout"}
        )
    ...
    parallel_calls = [
        ToolCall(tool="perplexity_search", namespace="sse", arguments={"query": f"Parallel test query {i}"})
        for i in range(3)
    ]
    ...
    try:
        ...
        for i, (_call, result) in enumerate(zip(parallel_calls, results, strict=False)):
        ...
        if wrapper_cls:
            try:
                ...
                if timeout_attrs:
                    ...
                    print("     No timeout attributes found")
