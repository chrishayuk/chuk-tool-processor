...
class TestSetupMCPSSE:
    ...
    async def test_setup_mcp_sse(self):
        ...
        with (
            ...
            patch("chuk_tool_processor.mcp.setup_mcp_sse.ToolProcessor", return_value=mock_processor),
        ):
            servers = [
            ...
            processor, stream_manager = await setup_mcp_sse(servers=servers, namespace="remote", enable_caching=True)
    ...
    async def test_setup_with_custom_options(self):
        ...
        with (
            patch("chuk_tool_processor.mcp.setup_mcp_sse.StreamManager.create_with_sse", AsyncMock()),
            patch("chuk_tool_processor.mcp.setup_mcp_sse.register_mcp_tools"),
            patch("chuk_tool_processor.mcp.setup_mcp_sse.ToolProcessor") as mock_processor_class,
        ):
