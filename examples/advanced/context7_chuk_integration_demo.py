...
class ResilientContext7Client:
    ...
    def _check_daily_reset(self):
        ...
        if today > self.last_reset_date:
            ...
            print("🌅 New day: Reset request counter and delays")
    ...
    async def _execute_with_retry(self, tool_name: str, **params) -> tuple[bool, str | None]:
        """
    ...
    async def search_library(self, library_name: str) -> str | None:
        """
        ...
        success, result = await self._execute_with_retry("resolve-library-id", libraryName=library_name, timeout=60.0)
        ...
        # Fallback
        print("   ⚠️ Search failed or no results, using fallback")
        return self._get_fallback_library_id(library_name)

    def _extract_library_id(self, response: str) -> str | None:
    ...
    def _get_fallback_library_id(self, library_name: str) -> str | None:
    ...
    async def get_documentation(
        self, library_id: str, topic: str = None, tokens: int = 8000, use_cache: bool = True
    ) -> str | None:
        """
        ...
        # Check daily limit
        if self._check_daily_limit():
            print("⚠️ Daily limit reached, skipping documentation request")
            return None
        ...
        params = {"context7CompatibleLibraryID": library_id, "tokens": tokens, "timeout": 90.0}
        if topic:
            params["topic"] = topic
        ...
        if success and result and len(result) > 100:
            ...
            # Cache the result
            if use_cache:
                self.documentation_cache[cache_key] = {"content": result, "timestamp": time.time()}

            return result
        else:
            print("   ❌ Documentation request failed or returned minimal content")
            return None

    async def get_library_documentation(
        self, library_name: str, topic: str = None, tokens: int = 8000, use_cache: bool = True
    ) -> str | None:
        """
...
async def example_resilient_workflow():
    ...
    async with ResilientContext7Client(
        ...
        react_docs = await client.get_library_documentation(
            "react", topic="useState useEffect useCallback useMemo", tokens=6000
        )
        ...
        vue_docs = await client.get_library_documentation("vue.js", topic="composition api setup", tokens=5000)
