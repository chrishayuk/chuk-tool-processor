...
class SSEHealthDiagnostic:
    ...
    async def _test_raw_sse_stream(self):
                ...
                async with client.stream('GET', self.gateway_url, headers=headers) as response:
                    ...
                    if response.status_code == 200:
                        ...
                        if 'text/event-stream' in response.headers.get('content-type', ''):
                            print(f"   ✅ Valid SSE stream established")
                            
                            # Read events for health analysis
                            print(f"   📡 Monitoring SSE events (15 second window)...")
        ...
        if stream_results["connection_successful"] and stream_results["session_discovered"]:
            print(f"   ✅ Raw SSE stream is healthy - connection issues likely in transport layer")
        elif stream_results["connection_successful"]:
            print(f"   ⚠️  SSE stream connects but no session discovery - may affect MCP protocol")
            self.health_issues.append("SSE stream missing session discovery")
        else:
            print(f"   ❌ Raw SSE stream has issues - this is the root cause")
        ...
        try:
            ...
            try:
                ...
                if init_success:
                    ...
                    if is_connected:
                        print(f"      ✅ Transport reports healthy")
                    else:
                        print(f"      ❌ Transport reports unhealthy")
                    ...
                    print(f"      ❌ Initialization failed - returned False")
                    self.health_issues.append("MCP initialization returned False")
            
            except asyncio.TimeoutError:
                handshake_results["timing"]["initialization"] = time.time() - init_start
                handshake_results["error"] = f"Initialization timeout after {handshake_results['timing']['initialization']:.1f}s"
    ...
    async def _investigate_transport_health(self):
        ...
        for attempt in range(3):
            ...
            try:
                ...
                health_investigation["health_consistency"].append({
                    "attempt": attempt + 1,
                    "error": str(e)
                })
        ...
        success_rate = (health_investigation["successful_initializations"] / 
                       health_investigation["initialization_attempts"] * 100)
    ...
    async def _analyze_health_reporting(self):
        ...
        analysis_results["raw_stream_healthy"] = (
            raw_sse.get("connection_successful", False) and 
            raw_sse.get("session_discovered", False)
        )
        
        analysis_results["mcp_handshake_successful"] = (
            mcp_handshake.get("initialization_completed", False) and
            mcp_handshake.get("message_url_obtained", False)
        )
        
        analysis_results["transport_reports_healthy"] = (
            mcp_handshake.get("health_check_passed", False)
        )
        
        print(f"   📊 Health Analysis Summary:")
        print(f"      Raw SSE stream: {'✅ Healthy' if analysis_results['raw_stream_healthy'] else '❌ Issues'}")
        print(f"      MCP handshake: {'✅ Successful' if analysis_results['mcp_handshake_successful'] else '❌ Failed'}")
        print(f"      Transport health: {'✅ Healthy' if analysis_results['transport_reports_healthy'] else '❌ Unhealthy'}")
        
        # Detect inconsistencies
        if (analysis_results["raw_stream_healthy"] and 
            analysis_results["mcp_handshake_successful"] and 
            not analysis_results["transport_reports_healthy"]):
            
            analysis_results["inconsistency_detected"] = True
            print(f"   🚨 INCONSISTENCY DETECTED: Stream works but transport reports unhealthy")
            ...
            print(f"   🔍 Root cause: Raw SSE stream has connectivity issues")
            analysis_results["likely_causes"].extend([
                "Network connectivity problems",
                "Authentication token issues",
                "SSE endpoint configuration problems"
            ])
            ...
            print(f"   🔍 Root cause: MCP protocol handshake failing")
            analysis_results["likely_causes"].extend([
                "MCP protocol version mismatch",
                "SSE Transport initialization bugs",
                "Session discovery timeout"
            ])
            ...
            print(f"   ✅ No inconsistencies detected - system appears healthy")
    ...
    async def _generate_resolution_report(self):
        ...
        print(f"  📋 MCP Handshake: {'✅ SUCCESSFUL' if mcp_handshake.get('initialization_completed') else '❌ FAILED'}")
        print(f"  🔧 Transport Health: {'✅ REPORTS HEALTHY' if mcp_handshake.get('health_check_passed') else '❌ REPORTS UNHEALTHY'}")
        print()
        ...
        # Success criteria
        success_score = 0
        if raw_sse.get("connection_successful"): success_score += 40
        if raw_sse.get("session_discovered"): success_score += 20  
        if mcp_handshake.get("initialization_completed"): success_score += 30
        if mcp_handshake.get("health_check_passed"): success_score += 10
