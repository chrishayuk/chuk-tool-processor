...
class CHUKProcessorDiagnostic:
    ...
    def __init__(self):
        ...
        self.servers = [
            {"name": "gateway", "url": self.base_url, "headers": {"Authorization": f"Bearer {self.auth_token}"}}
        ]
    ...
    async def _test_raw_transport(self):
        ...
        try:
            ...
            if init_success:
                ...
                print(
                    f"      {'✅' if ping_success else '❌'} Ping: {ping_success} ({transport_result['ping_time']:.2f}s)"
                )
    ...
    async def _test_stream_manager(self):
        ...
        try:
            ...
            stream_manager = await StreamManager.create_with_sse(
                servers=self.servers, connection_timeout=30.0, default_timeout=60.0, initialization_timeout=120.0
            )
            ...
            if hasattr(stream_manager, "transports"):
                for name, transport in stream_manager.transports.items():
                    ...
                    stream_manager_result["health_checks"].append({"transport": name, "healthy": is_healthy})
    ...
    async def _test_mcp_tool_registration(self):
        ...
        try:
            ...
            processor, stream_manager = await setup_mcp_sse(
                servers=self.servers, connection_timeout=30.0, default_timeout=60.0, namespace="gateway"
            )
    ...
    async def _analyze_root_cause(self):
        ...
        print(
            f"   Tool Execution: {execution.get('success_count', 0)}/{len(execution.get('tools_tested', []))} successful"
        )
