...
class MCPResilienceTestSuite:
    ...
    async def setup_test_environment(self):
        ...
        test_config = {
            "mcpServers": {
                ...
                "sqlite_test": {
                    "command": "uvx",
                    "args": ["mcp-server-sqlite", "--db", "/tmp/resilience_test.sqlite"]
                }
            }
        }
        ...
        try:
            ...
            for i in range(3):
                try:
                    call = ToolCall(
                        tool=test_tool,
                        arguments={"timezone": "Invalid/Timezone"} if "time" in test_tool else {}
                    )
            ...
            if test_result.recovery_count > 0:
                test_result.success = True
                self.print_progress(f"✅ Timeout recovery successful! {test_result.recovery_count}/3 recovery calls succeeded")
            else:
    ...
    def generate_test_report(self) -> Dict[str, Any]:
        ...
        for result in self.test_results:
            ...
            print(f"      Operations: {result.total_operations}, Errors: {result.error_count}, Recoveries: {result.recovery_count}")
            if result.details:
...
async def main():
    ...
    try:
        ...
        print(f"{Fore.YELLOW}This will test connection recovery, server crashes, timeouts, and concurrent stress.{Style.RESET_ALL}")
        print(f"{Fore.YELLOW}FIXED: Tool naming issues resolved - now uses actual tool names from registry{Style.RESET_ALL}")
