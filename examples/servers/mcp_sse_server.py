...
class MCPServerState:
    def __init__(self):
        self.sessions = {}
        self.server_info = {"name": "mcp-sse-demo-server", "version": "1.0.0"}
        self.capabilities = {
        ...
        self.tools = [
            ...
            },
            {
                ...
                "inputSchema": {
                    "type": "object",
                    "properties": {"query": {"type": "string", "description": "Fact query"}},
                    "required": ["query"],
                },
            },
            {
...
async def root():
    return {
        ...
        "version": "1.0.0",
        "endpoints": {"sse": "/sse", "messages": "/mcp"},
...
async def handle_mcp_message(request: Request):
    ...
    if not session_id:
        ...
        return {"jsonrpc": "2.0", "id": None, "error": {"code": -32602, "message": "Missing session_id parameter"}}

    if session_id not in state.sessions:
        logger.error(f"❌ Invalid session_id: {session_id}")
        logger.info(f"📊 Active sessions: {list(state.sessions.keys())}")
        return {"jsonrpc": "2.0", "id": None, "error": {"code": -32602, "message": f"Invalid session_id: {session_id}"}}
