    ...
    async def test_sse_connection_recovery(self) -> TestResult:
        ...
        try:
            ...
            initial_call = ToolCall(
                tool=test_tool,
                arguments={"query": "test connectivity"} if "search" in test_tool else {}
            )
            ...
            if test_tool in recovery_tools:
                recovery_call = ToolCall(
                    tool=test_tool,
                    arguments={"query": "test recovery"} if "search" in test_tool else {}
                )
    ...
    async def test_sse_dual_endpoint_coordination(self) -> TestResult:
        ...
        try:
            ...
            for i in range(5):
                call = ToolCall(
                    tool=test_tool,
                    arguments={"query": f"rapid test {i+1}"} if "search" in test_tool else {}
                )
            ...
            concurrent_calls = [
                ToolCall(
                    tool=test_tool,
                    arguments={"query": f"concurrent test {i}"} if "search" in test_tool else {}
                )
                for i in range(3)
            ]
        ...
        try:
            ...
            for i in range(3):
                try:
                    call = ToolCall(
                        tool=test_tool,
                        arguments={"query": "timeout test"} if "search" in test_tool else {}
                    )
            ...
            recovery_executor = ToolExecutor(
                registry,
            ...
            for i in range(3):
                recovery_call = ToolCall(
                    tool=test_tool,
                    arguments={"query": f"recovery test {i+1}"} if "search" in test_tool else {}
                )
            ...
            if test_result.recovery_count > 0:
                test_result.success = True
                self.print_progress(f"✅ SSE timeout recovery successful! {test_result.recovery_count} recovery calls succeeded")
            else:
    ...
    async def test_sse_concurrent_stress(self) -> TestResult:
        ...
        try:
            ...
            for result in stress_results:
                if result.error and any(keyword in str(result.error).lower() 
                                      for keyword in ["endpoint", "sse", "stream"]):
                    test_result.sse_specific_issues += 1
...
async def main():
    ...
    try:
        ...
        print(f"{Fore.YELLOW}Prerequisites: Start SSE server with 'python examples/test_sse_server.py'{Style.RESET_ALL}")
