...
from typing import Dict, List, Any, Optional, AsyncIterator, ClassVar
...
class LogStreamerTool(StreamingTool):
    ...
    async def _stream_execute(
        self, lines: int, interval: float, error_probability: float
...
class SearchTool(StreamingTool):
    ...
    async def _stream_execute(
        self, query: str, max_results: int, result_delay: float
    ) -> AsyncIterator[Result]:
        """
...
async def demonstrate_counter_tool(executor: ToolExecutor) -> None:
    ...
    for i, item in enumerate(results[0].result):
...
async def demonstrate_parallel_streaming(executor: ToolExecutor) -> None:
    ...
    calls = [
        ToolCall(
            tool="counter",
            namespace="stream",
            arguments={"start": 0, "end": 5, "delay": 0.5}
        ),
        ToolCall(
            tool="log_streamer",
            namespace="stream",
            arguments={"lines": 5, "interval": 0.6, "error_probability": 0.2}
        ),
        ToolCall(
            tool="search",
            namespace="stream",
    ...
    print(f"Parallel streaming saved approximately {sequential_time - total_time:.2f}s ({(sequential_time - total_time) / sequential_time * 100:.1f}%)")
