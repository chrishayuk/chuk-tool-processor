...
class HTTPStreamableResilienceTestSuite:
    ...
    async def discover_available_tools(self, namespace: str) -> List[str]:
        ...
        try:
            ...
            self.print_progress(f"Found {len(namespace_tools)} HTTP Streamable tools in namespace '{namespace}': {namespace_tools}")
    ...
    async def test_http_streamable_endpoint_recovery(self) -> TestResult:
        ...
        if not self.check_http_streamable_server_health():
            test_result.details["error"] = "HTTP Streamable server not running - start with: python examples/mcp_streamable_http_server.py"
        ...
        try:
            ...
            initial_call = ToolCall(
                tool=test_tool,
                arguments={"name": "connectivity test"} if "greet" in test_tool else {}
            )
            ...
            if test_tool in recovery_tools:
                recovery_call = ToolCall(
                    tool=test_tool,
                    arguments={"name": "recovery test"} if "greet" in test_tool else {}
                )
    ...
    async def test_http_streamable_session_management(self) -> TestResult:
        ...
        try:
            ...
            executor = ToolExecutor(
                registry,
                strategy=InProcessStrategy(registry, default_timeout=8.0)
            )
            ...
            for i in range(5):
                call = ToolCall(
                    tool=rapid_tool,
                    arguments={"name": f"rapid {i}"} if "greet" in rapid_tool else {}
                )
    ...
    async def test_http_streamable_response_modes(self) -> TestResult:
        ...
        try:
            ...
            executor = ToolExecutor(
                registry,
                strategy=InProcessStrategy(registry, default_timeout=10.0)
            )
            ...
            if quick_tools:
                ...
                for i in range(3):
                    call = ToolCall(
                        tool=quick_tools[0],
                        arguments={"name": f"immediate {i}"} if "greet" in quick_tools[0] 
                                 else {"increment": 1}
                    )
            ...
            if quick_tools:
                mixed_calls.append(ToolCall(
                    tool=quick_tools[0],
                    arguments={"name": "concurrent immediate"} if "greet" in quick_tools[0] 
                             else {"increment": 1}
                ))
        ...
        try:
            ...
            executor = ToolExecutor(
                ...
                strategy=InProcessStrategy(
                    registry,
                    default_timeout=10.0,
                    max_concurrency=self.config.concurrent_connections
                )
            )
    ...
    def generate_test_report(self) -> Dict[str, Any]:
        ...
        if passed_tests == total_tests and overall_success_rate >= 80:
            print(f"{Fore.GREEN}🎉 EXCELLENT: All HTTP Streamable tests passed with excellent performance!{Style.RESET_ALL}")
        ...
        print(f"   • Single endpoint architecture (vs SSE dual endpoints)")
        print(f"   • Optional streaming when needed (vs always-on SSE)")
        print(f"   • Better session management capabilities")
        print(f"   • Higher throughput potential")
        print(f"   • Modern infrastructure compatibility")
...
async def main():
    ...
    try:
        ...
        print(f"{Fore.YELLOW}Testing modern single-endpoint HTTP Streamable architecture (MCP spec 2025-03-26){Style.RESET_ALL}")
        print(f"{Fore.YELLOW}Prerequisites: Start HTTP Streamable server with 'python examples/mcp_streamable_http_server.py'{Style.RESET_ALL}")
