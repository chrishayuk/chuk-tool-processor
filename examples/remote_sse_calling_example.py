...
async def bootstrap_remote_mcp() -> None:
    ...
    if not servers:
        raise ValueError("No remote SSE MCP servers configured. Please set MCP_SERVER_URL or MCP_SERVER_URL_MAP environment variables.")
    ...
    try:
        ...
        print(f"❌ Could not connect to remote SSE server(s)")
...
async def run_remote_demo() -> None:
    ...
    for title, plugin, raw in plugins:
        ...
        try:
            if "perplexity_search" in raw and "perplexity_search" in available_tool_names:
                valid_plugins.append((title, plugin, raw))
            elif "perplexity_deep_research" in raw and "perplexity_deep_research" in available_tool_names:
                valid_plugins.append((title, plugin, raw))
            elif "perplexity_quick_fact" in raw and "perplexity_quick_fact" in available_tool_names:
                valid_plugins.append((title, plugin, raw))
        except Exception:
            pass
