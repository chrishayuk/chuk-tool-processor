...
class EnhancedSSEDiagnostic:
    ...
    def __init__(self):
        self.results = {"timestamp": time.time(), "tests": {}, "summary": {}, "version": "2.0 - Headers Support"}

    async def run_all_tests(self, url: str, headers: dict[str, str] = None, api_key: str = None) -> dict[str, Any]:
    ...
    async def _test_url_analysis(self, url: str):
        ...
        test_result = {"passed": True, "issues": [], "details": {}}
    ...
    async def _test_manual_http(self, url: str, headers: dict[str, str] = None, api_key: str = None):
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                logger.info(
                    "Request headers: %s", {k: v[:10] + "..." if len(v) > 10 else v for k, v in request_headers.items()}
                )
    ...
    async def _test_direct_sse_transport(self, url: str, headers: dict[str, str] = None, api_key: str = None):
    ...
    async def _test_stream_manager_integration(self, url: str, headers: dict[str, str] = None, api_key: str = None):
        ...
        try:
            ...
            server_config = {"name": "test_gateway", "url": url}
            ...
            stream_manager = await StreamManager.create_with_sse(
                servers=servers, connection_timeout=30.0, default_timeout=30.0
            )
    ...
    async def _test_headers_functionality(self, url: str, headers: dict[str, str] = None, api_key: str = None):
    ...
    def _generate_enhanced_summary(self):
        ...
        if manual_http.get("status_code") == 401:
            summary["recommendations"].append("❌ Authentication failed - check your API key and header format")
        elif manual_http.get("status_code") == 200:
            summary["recommendations"].append("✅ Manual HTTP connection works - issue may be in transport/integration")

        if manual_http.get("status_code") == 404:
            summary["recommendations"].append("❌ Endpoint not found - verify the SSE endpoint URL is correct")

        if not self.results["tests"].get("configuration", {}).get("server_found"):
            summary["recommendations"].append("⚠️ Gateway server not found in config - check your mcp_config.json")
        ...
        if summary["headers_status"] == "not_supported":
            ...
            summary["recommendations"].append("✅ Headers parameter supported - transport implementation is updated")
        ...
        if summary["integration_status"] == "failed":
            ...
            summary["recommendations"].append("✅ StreamManager integration working - full stack is functional")
