...
from chuk_tool_processor.execution.wrappers.retry import RetryConfig, RetryableToolExecutor, retryable
from chuk_tool_processor.execution.wrappers.rate_limiting import RateLimiter, RateLimitedToolExecutor, rate_limited
from chuk_tool_processor.execution.wrappers.caching import InMemoryCache, CachingToolExecutor, cacheable
...
async def retry_wrapper_demo(registry) -> None:
    ...
    high_fail_call = ToolCall(
        tool="flaky_api",
        namespace="demo",
        arguments={"fail_rate": 0.8, "data": "high failure test"}
    )
    ...
    low_fail_call = ToolCall(
        tool="flaky_api",
        namespace="demo",
        arguments={"fail_rate": 0.3, "data": "low failure test"}
    )
...
async def rate_limiting_wrapper_demo(registry) -> None:
    ...
    first_results = await executor.execute(calls[:2])
...
async def combined_wrappers_demo(registry) -> None:
    ...
    retry_executor = RetryableToolExecutor(
        executor=base_executor,
        default_config=RetryConfig(max_retries=2, base_delay=0.5)
    )
    ...
    combined_executor = RateLimitedToolExecutor(
    ...
    flaky_call = ToolCall(
        tool="flaky_api",
        namespace="demo",
        arguments={"fail_rate": 0.7, "data": "test combined"}
    )
